import bBtn from 'bootstrap-vue/src/components/button/button';
import bBtnClose from 'bootstrap-vue/src/components/button/button-close';
...
import { isVisible, selectAll, select, getBCR, addClass, removeClass, hasClass, setAttr, removeAttr, getAttr, hasAttr, eventOn, eventOff } from 'bootstrap-vue/src/utils/dom';
...
export default {
    ...
    render: function render(h) {
        ...
        if(!t.hideHeader) {
            ...
            if(!modalHeader) {
                ...
                modalHeader = [h(t.titleTag, {
                    class: ['modal-title']
                }, [$slots['modal-title'] || t.title]), closeButton];
            }
            header = h('header', {
                ref: 'header',
                class: t.headerClasses,
                attrs: {
                    id: t.safeId('__BV_modal_header_')
                }
            }, [modalHeader]);
        }
        // Modal Body
        ...
        if(!t.hideFooter) {
            ...
            footer = h('footer', {
                ...
                attrs: {
                    id: t.safeId('__BV_modal_footer_')
                }
            }, [modalFooter]);
        }
        ...
        if(!t.hideBackdrop && (t.is_visible || t.is_transitioning)) {
            backdrop = h('div', {
                ...
                attrs: {
                    id: t.safeId('__BV_modal_backdrop_')
                }
            });
        }
    ...
    computed: {
        dialogAttributes: function dialogAttributes() {
            let attributes = {
                id: this.safeId(),
                role: 'dialog',
            ...
            if(this.hideHeader) {
                // Allow aria-labelledby to be set manually
                if(this.ariaLabelledby !== '') {
                    attributes['aria-labelledby'] = this.ariaLabelledby;
                } else {
                    attributes['aria-label'] = this.$slots['modal-title'] || this.title;
                }
            } else {
                attributes['aria-labelledby'] = this.safeId('__BV_modal_header_');
            }
